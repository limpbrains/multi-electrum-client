...
- The two `ws`-server-backed files (`transport/ws.test.ts`, `client/electrum-client.ws.test.ts`) run against `test/rn/app/harness/ws-shim.ts`, a minimal RFC 6455 server on react-native-tcp-socket — the ws server runs on-device, so the full 26-file unit suite has parity.
- The integration suite also runs on-device (`suite-integration/` wrappers + `jest.harness.integration.config.mjs`): the device reaches the host's docker stack directly (iOS simulator via 127.0.0.1, Android emulator via 10.0.2.2); helpers are fetch-based and worked unmodified once `node:net`'s positional `createConnection(port, host)` form was added to the alias shim.
